...
This page is designed to explain how `?outermod` and `?outerbase` are designed to function for learning.
```{r setup}
library(outerbase)
```
...
The idea is to represent a surface as a realization of a Gaussian process controlled by a covariance function. An outer product of covariance functions can do this job in three dimensions.  This means we need to first build correlation functions.
```{r}
...
The goal is to predict at some number of points, say `1000` points.
```{r}
testsampsize = 1000
xtest = matrix(runif(testsampsize*d),ncol=d)
```

Then we built the predictor (assuming y is zero mean) using typical formulas.
```{r}
...
### outermod
An `outermod` instance is created using `new` command.
```{r}
om = new(outermod)
```
The first step is to set the covariances and the knots.  To set the vector of `covfs`, use `?setcovfs` alongside a vector strings of covariances in the package. The list can be found at `listcov()`. 
```{r}
setcovfs(om, rep("mat25",3))
```
This fixes the dimension of the `outermod` instance `om` to `3`.

Then we need to give it a set of knot points for each dimension.  The choice of these is still being researched, but choosing points that are spread out in each dimension that look like our actual responses is currently recommended.  You will need to invert a matrix of the size of these knot points, so it is recommended to keep it small, `<50` in general.  The function `?setknot` should be used.
```{r}
...
We can get an set our hyperparameters to align with our general Gaussian processes inference.
```{r}
gethyp(om)
...
### outerbase
An `outerbase` is the equivalent of a basis matrix with 
fast computation methods included.
```{r}
ob = new(outerbase, 
...
This basically just builds a set of basis functions for each dimension, which sometimes just look like polynomials.  They are not quite polynomials, and some `covf`s give different shapes.  The call `outermod$getbase` will allow you to access the basis functions for each dimension.
```{r}
...
### outermod and outerbase
`outermod` and `outerbase` are meant to be used in conjunction with each other.  One key ingredient is the `outermod$selecterms` function, which allows you to pick products of basis functions that best represent the current `outermod` response.  
```{r}
p = 60
terms = om$selectterms(p) # 60 by 3 matrix
head(terms)
```
`outermod$getvar` will tell us the vector of variances associated with these coefficients at these `terms`.
```{r}
covcoeff = as.vector(om$getvar(terms))
```

The specific basis matrix can be formed by getting the basis matrix at these selected `terms`.
```{r}
...
cbind(basevec[1:5],basismat[1:5,5]) #equal
```
...
To understand why this is valuable, we first show that if you take the `covcoeff` and `basismat` together, the correlation function is very well approximated through the following manipulation.
```{r}
