...
perturbvals = function(obj, coeff, coeffp,
  ...
  L$gradhyp = (L$gradhyp+L2$gradhyp+L_hyp$gradhyp)/3
  ...
  gradcheck$dhcoeff = cbind((L_hyp$diaghess - L$diaghess)/ep,
                            L$diaghessgradhyp %*% hypp)
  gradcheck$dhpara = cbind((L_para$diaghess - L$diaghess)/ep,
                          L$diaghessgradpara %*% parap)
  gradcheck
}
...
test_that("short, skinny test", {
  ...
  expect_equal_or_warnh(pinfo$prinfo$coeff[1], pinfo$prinfo$coeff[2], tolerance = 0.1,
               label="pr grad agreement")
  expect_equal_or_warnh(pinfo$prinfo$hyp[1], pinfo$prinfo$hyp[2], tolerance = 0.1,
               label="pr gradhyp agreement")
  expect_equal_or_warnh(pinfo$prinfo$para[1], pinfo$prinfo$para[2], tolerance = 0.1,
               label="pr gradpara agreement")
  
  expect_equal_or_warnh(pinfo$likinfo$coeff[1], pinfo$likinfo$coeff[2], tolerance = 0.1,
               label="lik grad agreement")
  expect_equal_or_warnh(pinfo$likinfo$hyp[1], pinfo$likinfo$hyp[2], tolerance = 0.1,
               label="lik gradhyp agreement")
  expect_equal_or_warnh(pinfo$likinfo$para[1], pinfo$likinfo$para[2], tolerance = 0.1,
               label="lik gradpara agreement")
  
  expect_equal_or_warnh(pinfo$vecinfo$coeff[1], pinfo$vecinfo$coeff[2], tolerance = 0.1,
                        label="lik grad agreement")
  expect_equal_or_warnh(pinfo$vecinfo$hyp[1], pinfo$vecinfo$hyp[2], tolerance = 0.1,
                        label="lik gradhyp agreement")
  expect_equal_or_warnh(pinfo$vecinfo$para[1], pinfo$vecinfo$para[2], tolerance = 0.1,
                        label="lik gradpara agreement")
  
  expect_equal_or_warnh(pinfo$likinfo$dhcoeff[,1], 
               pinfo$likinfo$dhcoeff[,2], tolerance = 0.1,
               label="lik dhgradcoeff agreement")
  expect_equal_or_warnh(pinfo$likinfo$dhpara[,1], 
               pinfo$likinfo$dhpara[,2], tolerance = 0.1,
               label="lik dhgradpara agreement")
  
  expect_equal_or_warnh(pinfo$likinfo$rep[1], 
               pinfo$likinfo$rep[2], tolerance = 10^(-12),
               label="rep agreement")
})
...
test_that("tall, skinny test", {
  ...
  expect_equal_or_warnh(pinfo$likinfo$rep[1], pinfo$likinfo$rep[2], tolerance = 10^(-12),
               label="rep agreement")
})
